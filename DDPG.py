...
if __name__ == "__main__":
    ...
    buffer_load_path = model_save_folder + "checkpoints/buffer.pth"
    # device = "cpu"
    device = 'cuda:0'
    torch.set_num_threads(48)
    showFig_iteration = 10  # save trajectories images
...
class DDPG:
    def __init__(
        ...
        # self.actor = DataParallel(Actor(state_dim, action_dim).to(device))
        # self.actor_target = DataParallel(Actor(state_dim, action_dim).to(device))
        # self.critic = DataParallel(Critic(state_dim, action_dim).to(device))
        # self.critic_target = DataParallel(Critic(state_dim, action_dim).to(device))
        self.actor = Actor(state_dim, action_dim).to(device)
        self.actor_target = Actor(state_dim, action_dim).to(device)
        self.critic = Critic(state_dim, action_dim).to(device)
        self.critic_target = Critic(state_dim, action_dim).to(device)
        writer.add_graph(self.actor, torch.rand(2).to(device), "Actor")
        writer.add_graph(self.critic,
                         (torch.rand(1, 2).to(device),
                          torch.rand(1, 2).to(device)), "Critic")
...
def train(env, agent: DDPG, episodes):
    ...
    for episode in range(episodes):
        ...
        if (episode + 1) % test_iteration == 0:
            pass
            # subprocess.Popen(["python", "../guidanceFit/netTest.py "],
            #                  shell=True)
            ...
            torch.save(
                {
                    ...
                    "model_state_dict_actor_target":
                        agent.actor_target.state_dict(),
                    "model_state_dict_critic_target":
                        agent.critic_target.state_dict(),
                    "optimizer_state_dict_actor":
                        agent.actor_optimizer.state_dict(),
                    "optimizer_state_dict_critic":
